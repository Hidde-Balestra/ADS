...
public class ClimateTracker {
    ...
    public Set<Station> getStations() {
        return Set.copyOf(stations.values());
    }
    ...
    public Map<Station,Integer> numberOfMeasurementsByStation() {
        Map<Station, Integer> measurementsMap = new HashMap<>();

        for (Station station: getStations()) {
            measurementsMap.put(station, station.getMeasurements().size());
        }
    ...
    public Map<Station,LocalDate> firstDayOfMeasurementByStation() {
       ...
       return stations.values().stream()
               .filter(station -> !station.firstDayOfMeasurement().equals(Optional.empty()))
               .collect(Collectors.toMap(station -> station, station -> station.firstDayOfMeasurement().get()));
    ...
    public Map<Station,Integer> numberOfValidValuesByStation(Function<Measurement,Double> mapper) {
        Map<Station, Integer> validValuesMap = new HashMap<>();

        for(Station station : getStations()){
                validValuesMap.put(station, station.numValidValues(mapper));
        }
    ...
    public Map<Integer,Double> annualAverageTemperatureTrend() {
        ...
        return getStations().stream()
                .flatMap(s -> s.getMeasurements().stream()).filter(m -> !Double.isNaN(m.getAverageTemperature()))
    ...
    public Map<Integer,Double> annualMaximumTrend(Function<Measurement,Double> mapper) {
        ...
        return getStations().stream()
                .flatMap(s -> s.getMeasurements().stream()).filter(m -> !Double.isNaN(mapper.apply(m)))
                .collect(Collectors.toMap((Measurement m) -> m.getDate().getYear(), mapper, Double::max));
    ...
    public Map<Month,Double> allTimeAverageDailySolarByMonth() {
        ...
        Map<Month, Double> averageDailySolarByMonth = getStations().stream()
                .flatMap(s -> s.getMeasurements().stream()).filter(m -> !Double.isNaN(m.getSolarHours()))
                .collect(Collectors.groupingBy(m -> m.getDate().getMonth(),
                        Collectors.averagingDouble(Measurement::getSolarHours)));

        averageDailySolarByMonth = new TreeMap<>(averageDailySolarByMonth);

        return averageDailySolarByMonth;
    }
    ...
    public int coldestYear() {
        Map<Integer, Double> coldestYearMap = getStations().stream()
                .flatMap(s -> s.getMeasurements().stream()).filter(m -> m.getMinTemperature() < 0)
                .collect(Collectors.groupingBy(m -> m.getDate().getYear(),
                        Collectors.summingDouble(Measurement::getMinTemperature)));


        double lowestValueFound = Collections.min(coldestYearMap.values());
        int yearOfLowestValue = -1;

        for (Map.Entry<Integer, Double> entry : coldestYearMap.entrySet()) {
            if(entry.getValue().equals(lowestValueFound)){
                yearOfLowestValue = entry.getKey();
                return yearOfLowestValue;
            }
        }
